...
describe('Hotels bookings', function () {
  ...
  BeforeEach()
  Before()
  it('POST /hotels/:address/confirmBooking. Expect 200', async () => {
    ...
    body = JSON.stringify({
      password: config.get('password'),
      reservationId: requests[0].dataHash
    })
    response = await fetch(`http://localhost:3000/hotels/${config.get('testAddress')}/confirmBooking`, {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
    ...
    expect(resp.requests).to.be.an('array')
  })
